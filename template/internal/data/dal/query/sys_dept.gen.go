...
func newSysDept(db *gorm.DB, opts ...gen.DOOption) sysDept {
	...
	_sysDept.DeletedBy = field.NewInt64(tableName, "deleted_by")
	_sysDept.CreatedAt = field.NewInt64(tableName, "created_at")
	_sysDept.UpdatedAt = field.NewInt64(tableName, "updated_at")
	_sysDept.DeletedAt = field.NewUint(tableName, "deleted_at")
	...
	_sysDept.Users = sysDeptHasManyUsers{
...
type sysDeptHasManyUsers struct {
	db *gorm.DB

	field.RelationField
}

func (a sysDeptHasManyUsers) Where(conds ...field.Expr) *sysDeptHasManyUsers {
	if len(conds) == 0 {
		return &a
	}
...
func (a sysDeptHasManyUsers) WithContext(ctx context.Context) *sysDeptHasManyUsers {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a sysDeptHasManyUsers) Session(session *gorm.Session) *sysDeptHasManyUsers {
	a.db = a.db.Session(session)
	return &a
}

func (a sysDeptHasManyUsers) Model(m *model.SysDept) *sysDeptHasManyUsersTx {
	return &sysDeptHasManyUsersTx{a.db.Model(m).Association(a.Name())}
}

type sysDeptHasManyUsersTx struct{ tx *gorm.Association }

func (a sysDeptHasManyUsersTx) Find() (result []*model.SysUser, err error) {
	return result, a.tx.Find(&result)
}

func (a sysDeptHasManyUsersTx) Append(values ...*model.SysUser) (err error) {
...
func (a sysDeptHasManyUsersTx) Replace(values ...*model.SysUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysDeptHasManyUsersTx) Delete(values ...*model.SysUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysDeptHasManyUsersTx) Clear() error {
	return a.tx.Clear()
}

func (a sysDeptHasManyUsersTx) Count() int64 {
	return a.tx.Count()
}
